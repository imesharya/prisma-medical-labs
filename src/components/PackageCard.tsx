...
أرجو التكرم بالتواصل معي للتأكيد النهائي.`
  ...
  return (
    <div
      key={pkg.id}
      className="group h-full bg-card border border-border rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col"
    >
      {/* Thumbnail — empty box when null */}
      ...
      <div className="p-6 flex flex-col gap-4 flex-1">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {hasCustomColor ? (
            <span
              className="inline-flex items-center gap-2 text-xs px-4 py-2 rounded-[50px] border border-solid text-nowrap"
              style={{
            ...
            <Button className="bg-primary/20 text-primary text-nowrap" size="sm" variant="outline">
              {pkg.badge || 'باقة'}
            </Button>
          )}
        </div>

        {/* Description — reserves 2 lines even when null */}
          ...
          {testCount > 0 && (
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col gap-0">
                <p className="font-bold text-lg">{testCount}</p>
        ...
        {/* Bottom actions — mt-auto pushes this to the bottom only when the card is stretched */}
        <div className="mt-auto flex flex-col gap-4">
          {testCount > 0 && (
            <Dialog>
              <DialogTrigger asChild>
                {hasCustomColor ? (
                  <button
                    className="w-full gap-2 rounded-lg border text-sm font-medium inline-flex items-center justify-center h-12 px-2.5 transition-all duration-200"
                    style={{
                  ...
                  <Button
                    variant="outline"
                    className="w-full gap-2 rounded-lg border-primary/20 bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground"
                  >
                    عرض التحاليل
                    <ChevronLeft />
                  </Button>
                ...
                <div className="p-6 pb-4 border-b border-border">
                  ...
                  <div className="flex items-center justify-start gap-3 mt-4">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 border border-border rounded-full">
                      <span className="text-xl font-bold text-foreground">{testCount}</span>
          ...
          <a href={buildWhatsAppLink(pkg.name)} target="_blank" rel="noopener noreferrer">
            {hasCustomColor ? (
              <button
                className="flex items-center w-full gap-2 rounded-lg text-white h-12 px-2.5 text-sm font-medium justify-center transition-all hover:brightness-110"
                style={{ backgroundColor: accentColor }}
              >
                احجز مجاناً الآن
              ...
              <Button className="flex items-center w-full gap-2 rounded-lg bg-primary text-primary-foreground">
                احجز مجاناً الآن
                <BsWhatsapp className="h-4 w-4" />
              </Button>
            )}
